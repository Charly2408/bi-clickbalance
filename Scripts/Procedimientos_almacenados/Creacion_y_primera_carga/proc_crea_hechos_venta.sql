...
CREATE PROCEDURE `proc_crea_hechos_venta`(IN flag bit(1), IN idEmpresa integer, IN baseDatosProd varchar(50), IN baseDatosBI varchar(50))
/*
Autor: Carlos Audelo
...
BEGIN
	...
	SET @query = CONCAT("INSERT INTO ",baseDatosBI,".fact_ventas(cliente_key, producto_key, agente_key, empresa_key,
		...
		FROM ", baseDatosBI, ".tmp_hechos_concentrado AS th 
		LEFT JOIN ", baseDatosBI,".agente AS a ON (th.agente_id = a.agente_nk) 
		LEFT JOIN ", baseDatosBI,".cliente AS c ON (th.cliente_id = c.cliente_nk) 
		INNER JOIN ", baseDatosBI,".empresa AS e ON (th.empresa_id = e.empresa_nk)
		INNER JOIN ", baseDatosBI,".info_movimiento AS im ON (th.tipo_venta_id = im.tipo_movimiento_nk AND th.estatus_venta = im.codigo_estatus) 
		INNER JOIN ", baseDatosBI,".info_pago AS ip ON (th.tipo_pago = ip.codigo_tipo_pago AND th.estatus_pago = ip.estatus_pago) 
		LEFT JOIN ", baseDatosBI,".moneda AS c ON (th.moneda_id = c.moneda_nk) 
		INNER JOIN ", baseDatosBI,".plaza AS pl ON (th.plaza_id = pl.plaza_nk)  
		LEFT JOIN ", baseDatosBI, ".producto AS p ON (th.producto_id = p.producto_nk)
		LEFT JOIN ", baseDatosBI,".territorio AS t ON (th.codigo_postal = t.codigo_postal) 
		LEFT JOIN ", baseDatosBI,".tiempo AS tmp ON (th.fecha_venta_id = tmp.fecha_key);");
    PREPARE myQue FROM @query;
    EXECUTE myQue;
